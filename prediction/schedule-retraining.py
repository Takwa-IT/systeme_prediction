...
class RetrainingOrchestrator:
    ...
    def fetch_real_values_data(self, days_back=30):
        ...
        try:
            ...
            logger.info(
                f"📥 Téléchargement des valeurs réelles depuis {days_back} jours...")
    ...
    def check_if_retraining_needed(self, real_values_data):
        ...
        if not real_values_data or len(real_values_data) < 20:
            logger.info(
                "⏭️ Pas assez de données réelles pour réentraînement (min 20)")
            return False
        ...
        if important_deviations / len(real_values_data) > 0.3:
            logger.info(
                f"⚠️ {important_deviations} déviations importantes détectées")
            return True

        logger.info(
            "✅ Performance acceptable, pas de réentraînement nécessaire")
        return False
        ...
        try:
            ...
            if existing_data_path.exists():
                ...
                df_combined = pd.concat(
                    [df_reals, df_existing], ignore_index=True)
                df_combined = df_combined.drop_duplicates(
                    subset=['execution_id'], keep='first')

            else:
                df_combined = df_reals

            # Sauvegarder dataset mixte
            mixed_dataset_path = DATA_DIR / \
                f"dataset_mixed_{datetime.now().strftime('%Y%m%d')}.csv"
            df_combined.to_csv(mixed_dataset_path, index=False)

            logger.info(
                f"✅ Dataset créé: {mixed_dataset_path} ({len(df_combined)} lignes)")
    ...
    def verify_model_update(self):
        ...
        try:
            ...
            if metadata_path.exists():
                ...
                if age_minutes < 5:
                    logger.info(
                        f"✅ Modèles mis à jour il y a {age_minutes:.0f} min")
                    return True
